...
namespace CloneBE.Application.Interface.Serivce
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<CategoryCreate> CreateCategory(CategoryCreate categoryDTO)
        {
            ...
            return mapper.Map<CategoryCreate>(categorydto);
        }
        ...
        {
            var existingCategory = await unitOfWork1.CategoryRepo.GetById(categoryDTO.CategoryId);
            if (existingCategory == null) return null;

            mapper.Map(categoryDTO, existingCategory); // Map đè lên object đang được EF theo dõi
            unitOfWork1.CategoryRepo.Update(existingCategory);
            await unitOfWork1.SaveChangesAsync();
